<script lang="ts">
    ...
    function selectFood(item: any) {
        ...
        userMacros = {
            ...
            carbohydrates:
                nutrientByName(item, "Carbohydrate, by difference") ?? 0,
        };
    ...
    function nutrientByName(item: any, nutrientName: string) {
        return (
            item.foodNutrients.find((n: any) => n.nutrientName === nutrientName)
                ?.value || "?"
        );
    }
    function viewMore() {
        ...
        <ul>
            {#each searchResults.slice(0, visibleCount) as item}
                <li on:click={() => selectFood(item)}>
                    <strong>{item.brandName ? item.brandName + ":" : ""}</strong
                    >
                    {item.description}<br />
                    Calories: {nutrientByName(item, "Energy")} kcal | Protein: {nutrientByName(
                        item,
                        "Protein",
                    )} g | Fat: {nutrientByName(item, "Total lipid (fat)")} g | Carbohydrates
                    {nutrientByName(item, "Carbohydrate, by difference")} g
                </li>
            {/each}
        </ul>
            ...
            <tbody>
                ...
                <tr
                    ><td>Calories</td><td
                        ><input
                            type="number"
                            bind:value={userMacros.calories}
                        /></td
                    ></tr
                >
                <tr
                    ><td>Protein (g)</td><td
                        ><input
                            type="number"
                            bind:value={userMacros.protein}
                        /></td
                    ></tr
                >
                <tr
                    ><td>Fat (g)</td><td
                        ><input type="number" bind:value={userMacros.fat} /></td
                    ></tr
                >
                <tr
                    ><td>Carbohydrates (g)</td><td
                        ><input
                            type="number"
                            bind:value={userMacros.carbohydrates}
                        /></td
                    ></tr
                >
            </tbody>
