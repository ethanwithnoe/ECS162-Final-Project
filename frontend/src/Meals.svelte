<script lang="ts">
    ...
    async function loadMeals() {
        try {
            const res = await fetch("/api/getuserfoods", {
                credentials: "include",
            });
                ...
                <tbody>
                    {#each filteredMeals as meal (meal._id)}
                        <tr>
                            ...
                            <td
                                ><span class="value">{meal.carbohydrates}</span
                                ></td
                            >
                            <td
                                ><button onclick={() => repeatFood(meal)}
                                    >+</button
                                ></td
                            >
                            <td
                                ><button onclick={() => removeFood(meal._id)}
                                    >-</button
                                ></td
                            >
                        </tr>
...
<style>
    /* Redoing the color scheme to match the Figma */
    .container {
